...
export default function Currencies() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://172.17.1.20:9090/api/allcurrency', {
                headers: {
    ...
    const handleSaveChanges = async (index) => {
        ...
        try {
            const response = await axios.put(`http://172.17.1.20:9090/api/updatecurrency/${updatedRows[index].id}`, updatedRows[index], {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                    "Accept": "application/json",
                    "Content-Type": "application/json"
