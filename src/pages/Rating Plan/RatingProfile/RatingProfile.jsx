...
export default function RatingProfile() {
    ...
    const handleConfirmDelete = () => {
        ...
        axios.delete(`https://bssproxy01.neotel.nr/abmf/api/rating/profile/delete/${recordIdToDelete}`, {
            headers: {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('https://bssproxy01.neotel.nr/abmf/api/prepaid/packs/get/all', {
                headers: {
                    Authorization: `Bearer ${tokenValue}`,
                    Accept: 'application/json',
                    'Content-Type': 'application/json',
    ...
    const SelectedRecordDetails = () => {
        ...
        if (selectedRecord) {
            return (
                <Grid>
                    <Paper elevation={10}>
                        <Card variant="outlined" sx={{ maxWidth: 450, fontFamily: "Roboto" }}>
                            <Box sx={{ p: 1 }}>
                            ...
                            <Grid container>
                                <Grid item xs={12} paddingLeft={1}>
                                    <Box sx={{ p: 1 }}>
                                        {[
                                            { label: "Plan Name", value: selectedRecord.plan_name },
                                            { label: "Pricing Model", value: selectedRecord.pricing_model },
                                            { label: "Voice Balance", value: `${selectedRecord.call_balance} ${selectedRecord.call_balance_parameter}` },
                                            { label: "SMS Balance", value: selectedRecord.sms_balance },
                                            { label: "Data Balance", value: `${selectedRecord.data_balance} ${selectedRecord.data_balance_parameter}` },
                                            { label: "Available Balance", value: selectedRecord.available_core_balance },
                                        ].map((field, index) => (
                                            <Box key={index} sx={{ p: 1 }}>
                                                <Grid container>
                                                    ...
                                                    <Grid item xs={4} sx={{ marginLeft: field.label === "SMS Balance" ? -12 : -6 }}>
                                                        <Typography sx={{ fontSize: '17px' }}>
                                                            {field.value}
                                                        </Typography>
                                                    </Grid>
                                                </Grid>
                                                {index !== 5 && <Divider light />}
                                            </Box>
                                        ))}
                                    </Box>
                                </Grid>
                            ...
                            <Grid container spacing={1} padding={1} sx={{ width: 400 }}>
                                <Grid item>
                                    {/* <Button
                                        ...
                                        onClick={() => navigate('/editTarrif', { state: { selectObj: selectedRecord } })}
                                    >
                                        Edit
                                    </Button> */}
                                </Grid>
                                <Grid item>
                                    <Button
                                        onClick={() => handleOpenConfirmationDialog(selectedRecord.rating_profile_id)}
                                        sx={{ backgroundColor: '#253A7D' }}
                                        variant="contained"
                                    >
                                        Delete
    ...
    const handleSerch = async (e) => {
        e.preventDefault();
        return await axios
            .get(`https://bssproxy01.neotel.nr/abmf/api/vendor/mgmt/detail/search?keyword=${value}`)
            .then((res) => {
                setdata(res.data);
        ...
        <Box sx={{ display: 'container', marginTop: -2.5, width: '79vw' }}>
            ...
            <Box sx={{ width: '70%', }}>
                ...
                <Box sx={{
                    ...
                    <Button
                        sx={{ backgroundColor: '#253A7D', boxShadow: 24 }}
                        variant="contained" backgroundColor="#253A7D" onClick={() => { navigate('/newTarrif') }}>
                        Add tarrif
                    </Button>
                </Box>
            </Box>
