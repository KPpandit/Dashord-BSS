...
export default function SimReports() {
    ...
    const fetchData = async () => {
        try {
            const response1 = await axios.get(`http://172.17.1.20:9098/sim/getall/sim/bydate`, {
                headers: {
            ...
            const response2 = await axios.get(`http://172.17.1.20:9098/sim/getall/sim/byType`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                    "Accept": "application/json",
                    "Content-Type": "application/json"
        ...
        try {
            const response =  await axios.get(`http://172.17.1.20:9098/sim/getall/sim/byStatus/type`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                    "Accept": "application/json",
                    "Content-Type": "application/json"
            ...
            console.log("Pie Chart Data:", pieChartData1);
        } catch (error) {
            console.error("Error fetching data:", error);
        }
    ...
    const handleCategory = async (categoryType) => {
        try {
            const response = await axios.get(`http://172.17.1.20:9098/sim/getall/sim/graph/bydateRange?startDate=${startdate}&endDate=${enddate}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                    "Accept": "application/json",
                    "Content-Type": "application/json"
                ...
                <Paper elevation={10}>
                    <Grid container spacing={5}>
                        ...
                        <Grid item xs={6}>
                            ...
                            <Chart
                                options={pieChartData1.options}
                                series={pieChartData1.series}
                                type="donut"
                                height={300}
                                width="100%"
                            />
