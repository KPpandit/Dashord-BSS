...
export default function EditAgent() {
    ...
    const { handleChange, handleSubmit, handleBlur, values, setValues: setValues1, submitForm: submaintAgentDetails } = useFormik({
        ...
        onSubmit: async (values) => {
            console.log("this is from the agent details")
            const res = await axios.put(`https://bssproxy01.neotel.nr/crm/api/updatepartner/${id}`,
                { ...values }, {
    ...
    const { handleChange: handleChange2, handleSubmit: handleSubmit2, handleBlur: handleBlur2,
        ...
        values: values2, setValues: setValues2, submitForm: submitMainForm2, } = useFormik({
            ...
            onSubmit: async (values2) => {
                ...
                const res2 = await axios.put('https://bssproxy01.neotel.nr/crm/api/updatePartnerCommission/' + id,
                    { ...values2 }, {
    ...
    useEffect(() => {
        const fetchCustomerDetails = async () => {
            try {
                const customerId = id; // Use the actual customer ID
                const response = await axios.get(`https://bssproxy01.neotel.nr/crm/api/partner/${customerId}`, {
                    headers: {
                        Authorization: `Bearer ${tokenValue}`,
                        Accept: "application/json",
                        "Content-Type": "application/json"
                ...
                if (response.status === 200) {
                    ...
                    console.log("Partner Commission Details: ", customerDetails.partnerCommission);

                    // Set the form values for commission details, handling null or undefined values
                    setValues2({
                        amount: customerDetails.partnerCommission?.amount ?? '', // Handle null or undefined with default ''
                    ...
                    console.log(values2,'---> values 2 value ')
                    // Handle other customer details for values1 if needed
                    setValues1({
                        ...
                        isActive: customerDetails.isActive || false
                    });

                    // Set Payment ID from partnerCommission
    ...
    }, [resetForm2, tokenValue, id]); // Ensure 'id' is in the dependency array
    // Add id if necessary

    // console.log(values,'values details')
    return (
                                ...
                                <Grid container spacing={2}>
                                    <Grid item xs={3}>
                                        <TextField
                                            ...
                                            label="Amount"
                                            name="amount"
                                            value={values2.amount || ''} // Handle null or undefined values
                                    ...
                                    <Grid item xs={3}>
                                        <TextField
                                            ...
                                            label="TYPE"
                                            name="type"
                                            value={values2.type || ''} // Handle null or undefined values
                                            onChange={handleChange2}
                                            onBlur={handleBlur2}
                                        />
                                    </Grid>
                                    <Grid item xs={3}>
                                        <TextField
                                            ...
                                            fullWidth
                                            name="partnerId"
                                            value={values2.partnerId || ''} // Handle null or undefined values
                                            onChange={handleChange2}
                                            onBlur={handleBlur2}
                                        />
                                    </Grid>
                                    <Grid item xs={3}>
                                        <TextField
                                            ...
                                            value={values2.commissionProcessRunId || ''} // Handle null or undefined values
                                            onChange={handleChange2}
                                            onBlur={handleBlur2}
                                        />
                                    </Grid>
