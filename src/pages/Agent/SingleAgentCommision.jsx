...
export default function SingleAgentCommission(props) {
    const location = useLocation();
    const { id } = location.state || {};
    console.log(id,"   daat of Agent")
    const agentId=id.id;
    const columns = [
        { id: 'Agent Id', name: 'ID' },
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('https://bssproxy01.neotel.nr/crm/api/commission/partner/'+agentId, {
                    headers: {
                ...
                <Box component="main" sx={{ flexGrow: 1, p: 1, width: '100%' }}>
                    <Paper elevation={10} sx={{ padding: 1,margin: 1, backgroundColor: 'white', color: '#253A7D',marginLeft:-1 }}>
                        <Grid>
                            ...
                            >Agent All Comission</Typography>
                        </Grid>
                    </Paper>
                </Box>
