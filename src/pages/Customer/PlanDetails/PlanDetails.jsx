...
export default function PlanDetails(selectedRecordId) {
    ...
    const { handleChange, handleSubmit, handleBlur, values, setValues } = useFormik({
        ...
        onSubmit: async (values) => {
            console.log(values);
            setResult(values);
            const res = await axios.post('http://172.5.10.2:9699/api/plan/allocation/postpaid/customer',
                { ...values, plan_id }, {
                headers: {
    ...
    const getApi = async (event) => {
        try {
            const response = await fetch(`http://172.5.10.2:9699/api/plan/allocation/assigned/postpaid/customer/${id}`);
            if (!response.ok) {
