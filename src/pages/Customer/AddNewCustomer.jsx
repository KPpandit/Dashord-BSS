...
export default function AddNewCustomer() {
  ...
  const { handleChange, handleSubmit, handleBlur, values } = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const res = await axios.post('https://bssproxy01.neotel.nr/crm/api/rates/offer/save', { ...values }, {
          headers: {
  ...
  const toAddCustomerDetails = () => {
    navigate("/subscriber/newSubscriber/addSubscriberDetails", { state: { accountType: values.is_rates_active } });
  };
